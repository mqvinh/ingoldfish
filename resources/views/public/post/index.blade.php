...
@foreach($lCatePost as $CatePost)
  <!-- blog list -->
  <div class="row">
    <div class="col-lg-4 col-sm-4 "><a href="{{ route('public.post.getPost', $CatePost['slug']) }}" class="thumbnail"><img style="width: 300px; height: 150px" src="{{ asset('/resources/upload/'.$CatePost['picture']) }}" alt="blog title"></a></div>
    <div class="col-lg-8 col-sm-8 ">
    <h3><a href="{{ route('public.post.getPost', $CatePost['slug']) }}">{{ $CatePost['name'] }}</a></h3>
    <div class="info">Ngày tạo: {{ $CatePost['date_create'] }}</div>                                               
    {{ $CatePost['preview'] }}
    <a href="{{ route('public.post.getPost', $CatePost['slug']) }}" class="more">Xem tất cả</a>
    </div>
  </div>
  <!-- blog list -->
@endforeach
